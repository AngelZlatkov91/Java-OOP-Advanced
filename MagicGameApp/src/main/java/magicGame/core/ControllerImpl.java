...
public class ControllerImpl implements Controller{
    ...
    public String addMagician(String type, String username, int health, int protection, String magicName) {
        Magician magician;
        Magic byName = this.magics.findByName(magicName);
        if (byName == null) {
            throw new NullPointerException(ExceptionMessages.MAGIC_CANNOT_BE_FOUND);
        }
        switch (type) {
            case "Wizard":
                magician = new Wizard(username,health,protection,byName);
                break;
            case "BlackWidow":
                magician = new BlackWidow(username,health,protection,byName);
                break;
